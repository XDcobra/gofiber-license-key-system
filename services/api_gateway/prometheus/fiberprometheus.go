...
type FiberPrometheus struct {
	...
	failedApiKeys   *prometheus.CounterVec
	defaultURL      string
}
...
func create(registry prometheus.Registerer, serviceName, namespace, subsystem string, labels map[string]string) *FiberPrometheus {
	...
	failedApiKeys := promauto.With(registry).NewCounterVec(
		prometheus.CounterOpts{
			Name:        prometheus.BuildFQName(namespace, subsystem, "apikeys_failed"),
			Help:        "Count all Requests made to an protected endpoint that failed due to an unauthorized api key",
			ConstLabels: constLabels,
		},
...
func (ps *FiberPrometheus) Middleware(ctx *fiber.Ctx) error {
	...
	if statusCode == "401" {
		ps.failedApiKeys.WithLabelValues(statusCode, method).Inc()
	}
